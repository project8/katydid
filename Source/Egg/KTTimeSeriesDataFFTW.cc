...
#ifdef ROOT_FOUND
    TH1C* KTTimeSeriesDataFFTW::CreateTimeSeriesHistogram(UInt_t channelNum) const
    {
        ...
        {
            hist->SetBinContent(iBin+1, (*(fChannelData[channelNum].fRecord))(iBin)[0]);
        }
        hist->SetXTitle("Time (s)");
        return hist;
    }

    TH1I* KTTimeSeriesDataFFTW::CreateAmplitudeDistributionHistogram(UInt_t channelNum) const
    {
        ...
        {
            hist->Fill(Double_t((*(fChannelData[channelNum].fRecord))(iBin)[0]));
        }
