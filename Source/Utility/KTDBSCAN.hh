...
namespace Katydid
{
    ...
    {
        LINFO(dbslog_h, "Initializing DBSCAN arrays with " << nPoints << " points");
    ...
    {
        LDEBUG(dbslog_h, "Starting DBSCAN; min points: " << fMinPoints << "; radius: " << fRadius);
        ...
        {
            //LWARN(dbslog_h, "Visiting point " << pid);
            // not already visited
            if (! fVisited[pid])
            {
                fVisited[pid] = true;
                ...
                //LWARN(dbslog_h, "This is a new point; it has " << ne.size() << " neighbors");

                // not enough support -> mark as noise
                if (ne.size() < fMinPoints)
                {
                    //LWARN(dbslog_h, "That's below the threshold; it's a noise point");
                ...
                else
                {
                    ...
                    //LWARN(dbslog_h, "Starting cluster with this point; cluster is " << cid);

                    // go to neighbors
                    for (unsigned int i = 0; i < ne.size(); ++i)
                    {
                        //LWARN(dbslog_h, "    Visiting neighbor, point " << ne[i]);
                        PointId nPid = ne[i];
                        ...
                        if (! fVisited[nPid])
                        {
                            ...
                            //LWARN(dbslog_h, "    This is a new point; it has " << ne1.size() << " neighbors");

                            // enough support
                            if (ne1.size() >= fMinPoints)
                            {
                                //LWARN(dbslog_h, "    Joining neighbor's neighbors with the previous cluster");
                                // join
                        ...
                        {
                            //LWARN(dbslog_h, "    Neighbor was not part of a cluster; adding to cluster " << cid);
