 ...
 *      Author: J. K. Gaison
 *      Based on KTChannelAggregator by P. T. Surukuchi
 */

#ifndef KTPHASEDAGGREGATOR_HH_
#define KTPHASEDAGGREGATOR_HH_
...
namespace Katydid
{
    ...
    /*
     ...
     Configuration name: "phased-aggregator"
     ...
     - "phases": vector<double> -- relative phases of separate channels in radians [given as <double>, <double>, ...]
     - "scales": vector<double> -- scaling coefficients for separate channels, set individual channels to 0.0 if not included [given as <double>, <double>, ...] 
    ...
    class KTPhasedAggregator : public Nymph::KTProcessor
    {
        public:
            KTPhasedAggregator(const std::string& name = "phased-aggregator");
            virtual ~KTPhasedAggregator();
            ...
            // Phase shifts and scale factors for complex signal summation
            // variables read in as strings from config file and cast into vector<double>
            MEMBERVARIABLE(std::string, Phase_str);
            MEMBERVARIABLE(std::string, Scale_str);
            MEMBERVARIABLE(std::vector<double>, Phases);
            MEMBERVARIABLE(std::vector<double>, Scales);
    ...
    inline bool KTPhasedAggregator::NullFreqSpectrum(KTFrequencySpectrumFFTW &freqSpectrum)
    {
